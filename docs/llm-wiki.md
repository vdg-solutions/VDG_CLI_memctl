...
// ~/.claude/settings.json
{
  "hooks": {
    "Stop": [{
      "matcher": "",
      "hooks": [{ "type": "command", "command": "memctl capture" }]
    }]
  }
}
```

`memctl capture` đọc Hook Protocol v1 payload từ stdin (session_id, cwd, transcript) — Claude Code Stop hook truyền vào. Zero-effort capture — bot không cần nhớ gì.

**Status:** Done (G1, backlog #11). `CaptureOperator` xử lý cả direct mode (`--role`/`--text`) và hook stdin mode.
...
|---|---|---|---|
| 🔴 P0 | Auto-capture (`capture` + Stop hook) | 1 task | Loại bỏ dependency vào bot nhớ lưu |
