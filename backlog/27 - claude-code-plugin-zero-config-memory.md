...
- **External repo CREATE (manual):** `rentaicoder/claude-plugins` — public repo serving as marketplace.
- **File CREATE in marketplace repo:** `.claude-plugin/marketplace.json`:
  ```json
  {
    "name": "rentaicoder",
    "description": "rentaicoder Claude Code plugins",
    "owner": {
      "name": "rentaicoder",
      "url": "https://github.com/rentaicoder"
    },
    "plugins": [
      {
        "name": "memctl",
...
claude plugin marketplace add rentaicoder/claude-plugins
claude plugin install memctl@rentaicoder
```
...
- Public repo `rentaicoder/claude-plugins` chưa tồn tại — task này tạo trong Step 5.
...
| Plugin name conflict với plugin khác tên `memctl` | Namespace bằng marketplace owner: install qua `memctl@rentaicoder`. |
...
- [USER-ACTION-REQUIRED] Tạo public repo `rentaicoder/claude-plugins` qua GitHub UI (https://github.com/new — owner: rentaicoder). Setting: Public, Empty (no README/license — bot init sau), Description: "rentaicoder Claude Code plugin marketplace". Paste back: repo URL.
- [USER-ACTION-REQUIRED] Cấp permission `rentaicoder/claude-plugins` cho `RELEASE_REPO_PAT` (đã có từ #25). Settings → Personal access tokens → edit existing PAT → add `rentaicoder/claude-plugins` to Repository access. Paste back: confirmation.
